    ...
    try {
        ...
        await database.subscription.update({
            ...
            data: {
                ...
                startDate: new Date(),
                nextBillingDate: await getNextBillingDate({ date: new Date(), interval: targetSubscription.interval })
            }
        });

        return NextResponse.json(apiResponsePresets.OK({ message: "Subscription activated successfully" }));
